...
void matr_mult_ellpack_V2(const ELLPACKMatrix *restrict matrix_a, const ELLPACKMatrix *restrict matrix_b, ELLPACKMatrix *restrict matrix_result)
{
    ...
    float *temp_b_row_values = (float *)calloc(matrix_b->num_cols, sizeof(float));

    uint64_t max_non_zero = 0;

    // Loop through each row of matrix_a
    for (uint64_t row_a = 0; row_a < matrix_a->num_rows; ++row_a)
    {
        matrix_result->result_values[row_a] = (float *)calloc(matrix_result->num_cols, sizeof(float));
        matrix_result->result_indices[row_a] = (uint64_t *)calloc(matrix_result->num_cols, sizeof(uint64_t));

        if (!matrix_result->result_values[row_a] || !matrix_result->result_indices[row_a])
        {
        ...
        for (uint64_t index_non_zero_a = 0; index_non_zero_a < matrix_a->num_non_zero; ++index_non_zero_a)

        {
            // Calculate the index of the current non-zero element in matrix_a
            uint64_t index_a = row_a * matrix_a->num_non_zero + index_non_zero_a;
            ...
            for (uint64_t num_non_zero_b = 0; num_non_zero_b < matrix_b->num_non_zero; ++num_non_zero_b)
            {
                ...
                temp_b_row_values[col_b] += matrix_b->values[index_b];
                // Store the column index in the temporary indices array
                matrix_result->result_indices[row_a][col_b] = col_b;
            }
            ...
            for (uint64_t col_b = 0; col_b < (matrix_b->num_cols - (matrix_b->num_cols % 4)); col_b += 4)
            {
                ...
                __m128 simd_values_b = _mm_load_ps(&temp_b_row_values[col_b]);
                // Load 4 values from the temporary values array into a SIMD register
                __m128 sind_temp_values = _mm_load_ps(&matrix_result->result_values[row_a][col_b]);
                ...
                _mm_store_ps(&matrix_result->result_values[row_a][col_b], sind_temp_values);
            }

            float a_value = matrix_a->values[index_a];
            ...
            {
                float calc_temp = temp_b_row_values[base_index_b + index_remain_b] * a_value;
                matrix_result->result_values[row_a][base_index_b + index_remain_b] += calc_temp;
            }

            // Reset the temporary B row array for the next iteration
            for (uint64_t col_b = 0; col_b < matrix_b->num_cols; ++col_b)
            {
                temp_b_row_values[col_b] = 0.0f;
            }
        }
        ...
        {
            if (matrix_result->result_values[row_a][index_col_result] != 0.0f)
            {
                // Store the value and index in the9 matrix_result matrix
                matrix_result->result_values[row_a][cnt_non_zero] = matrix_result->result_values[row_a][index_col_result];
                matrix_result->result_indices[row_a][cnt_non_zero] = matrix_result->result_indices[row_a][index_col_result];
                cnt_non_zero++;
            }
        }
